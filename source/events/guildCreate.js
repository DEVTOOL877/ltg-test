const { Client, Guild, MessageEmbed } = require("discord.js");
const db = require("quick.db");

/**
 *
 * @param {Client} client
 * @param {Guild} guild
 */

module.exports = async (client, guild) => {
  try {
    guild.channels
      .create("LTG_STUDIO 🎶", {
        type: "text",
        permissionOverwrites: [
          {
            id: guild.roles.everyone.id,
            deny: ["SEND_MESSAGES"],
            allow: ["VIEW_CHANNEL", "READ_MESSAGE_HISTORY", "ADD_REACTIONS"],
          },
        ],
        reason: "LTG_STUDIO",
        topic: "Play your favorite playlist with LTG_STUDIO 🎶",
      })
      .then((channel) => {
        let embed = new MessageEmbed()
          .setAuthor(
            "LTG_STUDIO",
            client.user.avatarURL({ dynamic: true }),
            "https://discord.gg/developer-support"
          )
          .setColor("BLUE")
          .setImage(
            "https://camo.githubusercontent."
          )
          .setTitle("**LTG_STUDIO Docs**")
          .setURL("https://ltg-studio.omarxg.repl.co/docs")
          .setTimestamp()
          .setDescription(`**Thanks For Using \`LTG_STUDIO\` Music Bot**
            
            > Developer: LTG_STUDIO
            `);
        channel.send({
          content: "> **Play your favorite playlist with LTG_STUDIO 🎶**",
          embeds: [embed],
          components: [],
        });
      })
      .catch((err) => {});
  } catch (err) {
    console.log(err);
  }
  require("../functions/guildCreateFunction")
    .get(guild, db)
    .catch((err) => console.log(err));
};
